...
End'''
    ...
    # regex = RegExParser("(.*|.*)*")
    # regex = RegExParser("(\\d{2}|\\d{4})[^0-9]+\\0")
    # regex = RegExParser("((.*)*)*")
    # regex = RegExParser(".*")
    # regex = RegExParser("(\\d{2,4}|\\[\\d+\\])")
    regex = RegExParser("^[A-Z][a-z]+$")
    ...
    if result:
        ...
        # interpreter = Interpreter(nfa, "<HTML>My birthday</HTML> is on <DATE>1977-09-14</DATE>.人人生MyMy14 1977")
        interpreter = Interpreter(nfa, test_text)
